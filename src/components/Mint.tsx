...
import { AddressExplorer, TransactionExplorer } from "./Explorer";
import { USDX_DECIMALS } from "~/const/ofts";

interface IMintTransfer {
  inputAddress: string;
  amount: string;
...
export const Mint = () => {
  ...
  const [transfers, setTransfers] = useState<IMintTransfer[]>([]);
  ...
  const handleFormSubmit = (event: React.FormEvent) => {
    ...
    if (selectedWallet && inputAddress && amount && userAddress) {
      getOFTContract(selectedWallet).then((contract) => {
        if (contract) {
          const transfer: IMintTransfer = {
            inputAddress,
            amount,
          };
          setTransfers([transfer, ...transfers]);
          contract.mint(inputAddress, toBaseUnit(amount, USDX_DECIMALS).toString(10)).then((txn) => {
        ...
        <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
          <div className="rounded-xl bg-white p-4 shadow sm:p-7 dark:bg-neutral-900">
            <div className="mb-8 text-center">
              <h2 className="text-2xl font-bold text-gray-800 md:text-3xl dark:text-neutral-200">Mint</h2>
              <p className="text-sm text-gray-600 dark:text-neutral-400">Mint OFT to target address</p>
            </div>
                  ...
                  {transfers.map((_, index) => (
                    <tr className="border-b bg-white dark:border-neutral-700 dark:bg-gray-800">
                      ...
                      <td className="px-6 py-4">
                        <p>
                          <AddressExplorer chainId={chainId} address={_.inputAddress} />
                        </p>
                        <p>
